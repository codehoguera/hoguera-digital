...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            ...
            <form action="{{ route('users.students.student') }}" class="d-flex mb-2 form-search-index"  method="POST">
                @csrf
                <input class="form-control me-2" type="search" placeholder="Buscar por nombre" aria-label="Search" name="query">
                <button class="btn btn-outline-success" type="submit">Search</button>
            </form>
            <form action="{{ route('users.students.studentbysearch') }}" class="d-flex mb-2 form-search-index"  method="POST">
                @csrf
